var PassportModule = (function () {
	var capturedImage = null; // The current captured image.
	...
	var passport_text = "Passport";

	var serverSettingsOptions = {
		serverType: "RTTI",
		//MobileID 2.0
		serverURLRTTIForConcerto: "https://use_a_valid_URL_instead_of_this",
	...
	var initializeExtractionResultsforStandalone = function () {
		...
		for (var i = 0; i < extractedResults.length; i++) {
			...
			if (extractedResults[i].name !== FIELD_FACEIMAGE64 && extractedResults[i].name !== FIELD_SIGNATUREIMAGE64) {
				...
				if (extractedResults[i].name == FIELD_PRODUCT_VERSION) {
					...
					li.push(
						"<li><input type=" +
							...
							value +
							'" onchange="PassportModule.updateValues(false,this.value,' +
							i +
							')"/></li>'
					);
				}
	...
	var initializeServerSettingsScreenEvents = function () {
		...
		$("#ProcessIdentity").val(serverSettingsOptions.processIdentityNameForConcerto);
	...
	// Saves the changes made for Server Settings
	var saveServerSettings = function () {
		serverSettingsOptions.serverType = $("input[name=radio-choice]:checked", "#ServerType").val();

		if (serverSettingsOptions.serverType == "RTTI") {
			serverSettingsOptions.serverURLRTTIForConcerto = $("#serverurl").val();
		} else {
			serverSettingsOptions.serverURLKTAForConcerto = $("#serverurl").val();
			serverSettingsOptions.ktaUserNameForConcerto = $("#ktausername").val();
			serverSettingsOptions.ktaPasswordForConcerto = $("#ktapassword").val();
			serverSettingsOptions.ktaSessionIdForConcerto = $("#ktasessionId").val();
			serverSettingsOptions.processIdentityNameForConcerto = $("#ProcessIdentity").val();
		}
	};
	...
	var performRTTIExtraction = function () {
		...
		rttiOptions.url = serverSettingsOptions.serverURLRTTIForConcerto;
		...
		function parserSuccessCallBack(parsedResults) {
			if (parsedResults[0].fields != null) {
				handleRTTIParsedResponse(parsedResults, rttiOptions);
			} else {
				Utils.hideLoadingIndicator();
				handleExtractionError({ code: parsedResults[0].errorCode, message: parsedResults[0].errorDescription });
			}
		}
		...
		if (currentpage == "previewscreen" && capturedImage) {
			...
			if (parsedResults[0].result != "Error") {
				...
				if (!isOnBoardingFlow) {
					...
					NewAccountModule.setExtractionInformationFromExtractionResults(extractedResults);
	...
	var performKTAExtraction = function () {
		...
		ktaOptions.serverParameters.username = serverSettingsOptions.ktaUserNameForConcerto;
		ktaOptions.serverParameters.password = serverSettingsOptions.ktaPasswordForConcerto;
		ktaOptions.serverParameters.sessionId = serverSettingsOptions.ktaSessionIdForConcerto;
		ktaOptions.serverParameters.processIdentityName = serverSettingsOptions.processIdentityNameForConcerto;

		ktaOptions.images = base64Array;
		ktaOptions.serverParameters.documentGroupName = "";
		ktaOptions.serverParameters.documentName = "";
		ktaOptions.serverParameters["IDType"] = "Passport";
